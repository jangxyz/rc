...
cat package.json | npx jqn 'update("scripts.preview", v => v === "vite preview" ? "dotenv -c -- bash -c '\''npx vite preview --port ${PORT:-5173}'\''" : v)' > package.json.setup-pnpm-vite-port-env
